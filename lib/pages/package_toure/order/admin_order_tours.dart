...
import 'package:chanthaburi_app/pages/package_toure/order/order_tour.dart';
...
class OrderPackageTours extends StatefulWidget {
  OrderPackageTours({Key? key}) : super(key: key);

  @override
  State<OrderPackageTours> createState() => _OrderPackageToursState();
}

class _OrderPackageToursState extends State<OrderPackageTours> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(isFilter ? "คัดกรองออร์เดอร์" : "ออร์เดอร์คำสั่งซื้อ"),
        actions: isFilter
            ? [
                IconButton(
                  onPressed: () => setState(() {
  ...
  StreamBuilder<QuerySnapshot<BookingTourModel>> buildStreamOrders(
      double width, double height) {
    return StreamBuilder(
        stream: OrderTourCollection.orderByAddmin(
            orderCategory, dateCreate!, endDate!),
        builder: (context,
            AsyncSnapshot<QuerySnapshot<BookingTourModel>> orderSnapshot) {
          ...
          List<QueryDocumentSnapshot<BookingTourModel>> orders =
                  ...
                  InkWell(
                    onTap: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => OrderTour(order: orders[index]),
                        ),
                      );
                    },
                    child: Container(
                      ...
                      child: Row(
                        children: [
                          buildDetail(
                            ...
                            orders[index].data().adult +
                                orders[index].data().senior +
                                orders[index].data().youth,
                            orders[index]["status"],
